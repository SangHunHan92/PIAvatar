...
# NOTE: decode_param_separable_contact does `from mpm_solver_warp...separable_contact import MPM_Simulator_WARP`
# at module scope, so its `import *` would clobber any earlier supertag import.
# Re-import the supertag solver AFTER decode_param to make sure the supertag class wins.
import warp as wp
...
# Utils
# super-tag self-contact: use the supertag variant of decode_param so that
# modify_smplx / modify_animatable_gaussians (which contain nested @wp.kernels
# bound to MPMStateStruct) point at the supertag struct (with particle_supertag).
# Re-importing supertag MPM_Simulator_WARP after decode is also safe since
# decode_param_supertag_contact already imports the supertag solver.
from utils.decode_param_lbs_decouple import *
from mpm_solver_warp.mpm_solver_warp_lbs_decouple import MPM_Simulator_WARP
...
if __name__ == "__main__":
    ...
    if sim_params["output_path"] is not None:
        ...
        copy_files = ["mpm_solver_warp/mpm_solver_warp_lbs_decouple.py", "mpm_solver_warp/warp_utils_lbs_decouple.py",
                      "mpm_solver_warp/mpm_utils_lbs_decouple.py", "mpm_solver_warp/mpm_human_utils_lbs_decouple.py",
                      "utils/decode_param_lbs_decouple.py", "utils/dataset.py", "utils/render_utils.py", "AnimatableGaussians/main_avatar_phys.py",
                      "simulation_lbs_decouple.py"]
    ...
    # LBS-decouple: push human_sequences-derived per-particle LBS into mpm_state
    # so the very first p2g2p call has correct super-tag classification.
    # When sim_params["use_lbs_decouple"] is False (A/B baseline), skip the LBS
    # push: particle_supertag stays at id-only seeding (one super-tag per
    # subject), so per-supertag grid_vk effectively collapses to per-subject
    # behavior — equivalent to base-MPM kinematic decomposition.
    if sim_params.get("use_lbs_decouple", True):
        mpm_solver.fill_particle_lbs_from_human_sequences(human_sequences, init_gs_nums, device=device)
        mpm_solver.recompute_super_tags_from_lbs(device=device)
    else:
        print("[lbs_decouple] use_lbs_decouple=False → skipping LBS push (per-subject baseline)", flush=True)
        ...
        if sim_params["render_img"]:
            ...
            if 1:
                ...
                rendering, raddi = rasterize(
                    means3D=pos,
                    means2D=mpm_params["screen_points"][:init_gs_num] if mpm_params["screen_points"].shape[0] > init_gs_num else mpm_params["screen_points"],
                    shs=shs, # shs
