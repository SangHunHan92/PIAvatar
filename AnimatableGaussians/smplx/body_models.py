...
class SMPL(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        vertices, joints, A, _ = lbs(betas, full_pose, self.v_template,
...
class SMPLX(SMPLH):
    ...
    NUM_BODY_JOINTS = SMPLH.NUM_BODY_JOINTS # SMPL.NUM_JOINTS - 2 = 21
    NUM_HAND_JOINTS = 15
    NUM_FACE_JOINTS = 3
    ...
    def forward(
        ...
        **kwargs
        ...
        # joint가 127개인 이유는 face때문이다, 이거 끄면 55개
        # SMPL 선언할때 face conqour 끄면 55개
        # A = rotation matrix이다
        # 사람은 관절만 있어서 rotation만 있으면 움직인다 -> 4x4 matrix
        # self.use_face_contour = False로 하기
        
        # self.J_regressor를 통해서 keypoint가 나오고 A pose가 곱해지면 keyponit가 움직인다
        # self.J_regressor : [55, 10475], vertices : torch.Size([1, 10475, 3])
        # 1. smpl vertices랑 J_regressor 추가하던가, keypoint 추가하기
        # 2. keypoint 위치로 smpl param 찾을 수 있나 ?
        
        # joints : torch.Size([1, 55, 3]), posed joints
        # joints = J_regressor @ vertices ?? No !!
        # verts, J_transformed : posed joints, A, J : default joints
        vertices, joints, A, J, transforms = lbs(shape_components, full_pose, self.v_template,
        ...
        # Add any extra joints that might be needed
        joints = self.vertex_joint_selector(vertices, joints) # 55 -> 76
        # Add the landmarks to the joints
        joints = torch.cat([joints, landmarks], dim=1) # 76 -> 127
        # Map the joints to the current dataset
        ...
        # joints : torch.Size([1, 127, 3]), the same [:, :55, :]
        v_shaped = None
        if return_shaped:
            v_shaped = self.v_template + blend_shapes(betas, self.shapedirs)
